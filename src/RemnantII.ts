...
export default class RemnantII implements types.IGame {
    ...
    async queryPath() {
        const game: types.IGameStoreEntry = await util.GameStoreHelper.findByAppId([STEAMAPP_ID]);
    ...
    async setup(discovery: types.IDiscoveryResult) {
        try {
            await fs.ensureDirWritableAsync(path.join(discovery.path, SCRIPT_MODS_PATH));
            await fs.ensureDirWritableAsync(path.join(discovery.path, PAK_MODS_PATH));
        }
        catch (err) {
            return Promise.reject(new Error('Unable to write to Mods folder for Remnant II: ' + err.message));
        }

        // Check if Allow Asset Mods (RE-UE4SS) is installed/deployed.
        const allowModsPath = path.join(discovery.path, 'Remnant2', 'Binaries', 'Win64', XINPUT_DLL);
        try {
