...
async function testModDir(
  ...
  const pakFile = files.find(
    (file) => !file.includes("/") && file.endsWith(PATTERN_PAK_MOD),
  );
...
async function installModDir(files: string[]): Promise<types.IInstallResult> {
  ...
  if (!contentFile)
    throw new Error(
      'Could not install mod as it does not include a ".pak" file.',
    );
  ...
  const filtered = files.filter(
    (file) => !file.endsWith(path.sep) && path.extname(file) !== ".txt",
  );
