...
function main(context: IExtensionContext) {
    ...
    const getDiscoveryPath = (): string => {
        ...
        if (!discovery || !discovery.path) {
            log('error', 'Remnant II game path could not be found as the game is not discovered');
            return undefined;
        }
    ...
    context.registerInstaller('r2-pak-installer', 50, testModDir, (files, destinationPath) => installModDir(context, files, destinationPath));

    // Allow Asset Mods (AAM) is a mod that allows other mods to be loaded.
    context.registerInstaller('r2-aam-installer', 30, testAAM, (files, dest) => installAAM(getDiscoveryPath, files, dest));
    context.registerModType('AAM', 30, gameId => gameId === GAME_ID, getDiscoveryPath, isAAMModType);

    // Other mods that need to be added to the root folder
    context.registerInstaller('r2rootDir', 50, testRootDir, installRootDir);
    context.registerModType('r2rootDir', 25, (gameId) => (gameId === GAME_ID), () => getDiscoveryPath(), isRootDirMod);
